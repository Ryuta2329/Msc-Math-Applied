...
```{r p02-01-01, anchor="figura", fig.cap="Serie temporal para los Pasajeros de la Clase Económica: Melbourne-Sydney."}
autoplot(jj) +
...
df_ts <- ts(cbind(part, tempr, cmort), 
	start=start(part), 
...
$$Mt = `r round(coef_matrix[1,1], 1)` `r round(coef_matrix[2,1], 3)`t  `r round(coef_matrix[3,1], 3)`(T_t − T_{.}) + `r round(coef_matrix[4,1], 3)`(T_t − T_{.})^2 + `r round(coef_matrix[5,1], 3)`P_t + w_t$$
