...
glance(all_models) %>%
  ...
  kbl(digits=c(NA, 4, 2, 1, 1, 1, 4, 4, 4), 
...
cm_transf %>%
  ...
  mutate(
    ...
    firs_half=ifelse(month <=6, "First Half", "Second Half"),
    quarter=lubridate::quarter(index)
  ) %>%
  group_by(firs_half) %>%
  summarise(month_mean=mean(prop_change))
```
