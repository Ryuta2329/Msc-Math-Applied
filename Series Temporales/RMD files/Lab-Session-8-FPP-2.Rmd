...
source(here::here("..", "assets", "caption-hooks.R"))
```
...
Dado los resultados encontrados para el primer modelo ajustado, se decide hacer un análisis de correlación entre la serie temporal con respecto a las series temporales retrasadas (`r figr("bivariate-lag", TRUE, type = "figura")`).
...
```{r lagged-regression, anchor="tabla"}
...
Los gráficos de residuales se muestran en la `r figr("diagnostics-plots-2", TRUE, type = "figura")` para el modelo de la ecuación <a href="#eq:model-reg">2</a>, con la dependencia cuadrática (el cual se selecciona al revisar los residuales). 
...
```{r diagnostics-plots-2, fig.width=20, fig.height=5, fig.cap="Gráficos diagnósticos de residuales para el modelo regresivo con la dependencia cuadratica con $r_{t-5}$: _a)_ ACF, _b)_ PACF, _c)_ gráficos de residuales, y _d)_ gráfico _QQ_", anchor="figura"}
...
Los parametros estimados para el modelo escogido (de la ecuacion <a href="#eq:model-reg">2</a>) se muestran en la `r figr("parametters-table", TRUE, type="tabla")`. 
